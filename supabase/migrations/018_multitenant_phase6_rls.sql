-- Fáze 6 multi-tenant migrace: SECURITY DEFINER helper funkce a workspace-scoped RLS politiky.
-- Tuto migraci je nutné nasadit SPOLEČNĚ s nasazením nového aplikačního kódu (Fáze 6).
-- Stará RLS (user_id-based) se odstraní a nahradí workspace-based politikami.

-- ─── Pomocné SECURITY DEFINER funkce ────────────────────────────────────────
-- SECURITY DEFINER = funkce čte workspace_members / profiles mimo RLS (žádná rekurze).
...
-- Accountant policy nemusí existovat (nebyla v migraci), IF EXISTS je bezpečné.
...
-- ─── RLS: document_items ─────────────────────────────────────────────────────
-- document_items nemá přímý workspace_id — izolace jde přes JOIN na documents.
...
-- ─── RLS: reminders ──────────────────────────────────────────────────────────
-- reminders nemá přímý workspace_id — izolace jde přes JOIN na documents.
